module Refinery
  ...
  protected
    ...
    def write_cache?
      if ::Refinery::Setting.find_or_set(:cache_pages_full, false,
        :restricted => true, :value => false, :form_value_type => 'check_box')
          cache_page(response.body,
            File.join('', 'refinery', 'cache', 'pages', request.path).to_s)
      end
    end
  end
end
