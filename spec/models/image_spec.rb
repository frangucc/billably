...
describe Image do
  ...
  describe 'Class Methods' do 
    it 'self.process_in_background should process image in background' do
      ...
      image.should_receive(:remote_attachment_net_url=).with(path).and_return(path)
  ...
  describe "Instance Methods" do
    describe "save_and_process_attachment" do
      ...
      it "should process intermediately" do
        path = 'path-to-image' 
        @image.stub!(:remote_attachment_net_url).and_return(path)
        @image.should_receive(:remote_attachment_url=).with(path).and_return(path)
      ...
      it "should call delay job worker" do
        path = 'path-to-image' 
        @image.stub!(:remote_attachment_net_url).and_return(path)
        @image.should_receive(:remote_attachment_net_url=).with(nil).and_return(nil)
